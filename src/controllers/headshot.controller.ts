...
import { Headshot } from "@/model/Headshot.modal";
import { headshotService, HeadshotStyle } from "@/services/headshot";
import { triggerHeadshotGeneration } from "@/services/queue";
import { s3Service } from "@/services/s3";
import { AppError, InsufficientCreditsError } from "@/util/error";
...
export const getAvailableStyles = async (req: Request, res: Response) => {
    ...
    const getAvailableStyles = headshotService.getAvailableStyles();

    return successResponse(res, "Available styles fetched successfully", getAvailableStyles);
}
